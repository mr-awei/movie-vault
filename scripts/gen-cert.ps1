...
if (-not $env:yinghai_CERT_PASSWORD) {
  Write-Error "请先设置环境变量 yinghai_CERT_PASSWORD（用于导出 pfx 的密码），例如：`$env:yinghai_CERT_PASSWORD='你的随机强密码'"
  exit 1
}
$password = $env:yinghai_CERT_PASSWORD
...
Write-Output "pfx 已导出: build\yinghai-sign.pfx （密码取自环境变量 yinghai_CERT_PASSWORD）"
Write-Output ""
